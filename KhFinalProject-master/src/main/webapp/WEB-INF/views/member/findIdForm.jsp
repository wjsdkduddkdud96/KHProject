...
<form class="signUp" id="signupForm">
   <h1 class="signUpTitle">ID 찾기</h1>
   <input type="text" class="signUpInput" name="nameSearch" id="nameSearch"  placeholder="이름"/>
   <input type="email" class="signUpInput" name="emailSearch" id="emailSearch" placeholder="이메일"/>
   <input type="button" id="find" type="button" value="찾기" class="signUpButton">
   <br/>
   <br/>
   <span id="idresult" class="btn btn-primary btn-block btn-large"></span>
</form>
	               ...
	               contentType: "application/json; charset=UTF-8",
	                 success : function(data) {
	                   //alert(data.findId);
	                    $("#idresult").empty();
	                    $("#idresult").hide();
	                    $("#idresult").fadeIn(1000);
	                    $("#idresult").append(data.findId);
	               	},
	               error : function(error) {
	                   //alert("다시 작성해주세요.");
	                   $("#idresult").empty();
	                   var txt = "일치하는 정보가 없습니다.";
	            	   //$("#idresult").append(txt);
	            	  // $(txt).appendTo('#idresult').fadeIn("slow");
	            	   //txt.hide().fadeIn(2000);
	            	  /*  $(function(){
	            		   var ex_show_3000 = $("#find");
